...
public class GBM {
	...
	public boolean run() throws IOException {
		...
		testing.write("Ref_Allele" + "," + "Alt_Allele" + ",");
		testing.write("Father_Allele_Balance" + "," + "Father_Read_Depth" + ",");
		testing.write("Father_Mean_Mapping_Quality_For_Ref" + "," + "Father_Mean_Mapping_Quality_For_Alt" + ","
		...
		testing.write("Father_Fraction_Of_Soft_Clipped_Reads_For_Ref" + ","
		...
		testing.write("Mother_Mean_Mapping_Quality_For_Ref" + "," + "Mother_Mean_Mapping_Quality_For_Alt" + ","
		...
		testing.write("Mother_Fraction_Of_Soft_Clipped_Reads_For_Ref" + ","
		...
		testing.write("Offspring_Mean_Mapping_Quality_For_Ref" + "," + "Offspring_Mean_Mapping_Quality_For_Alt" + ","
				+ "Offspring_Mean_Distance_To_Three_Prime_For_Ref" + ","
				+ "Offspring_Mean_Distance_To_Three_Prime_For_Alt" + ",");
		testing.write(
				"Offspring_Fraction_Of_MQ0_Reads_For_Ref" + "," + "Offspring_Fraction_Of_MQ0_Reads_For_Alt" + ",");
		testing.write("Offspring_Fraction_Of_Soft_Clipped_Reads_For_Ref" + ","
		...
		testing.write("Homopolymer_Flag" + "," + "Short_Tandem_Repeat_Flag" + "\n");
		...
		while (dnmRecord != null) {
			...
			while ((tmp = trioSamLocusIterator.getLocusInfos()) != null) {
				...
				testing.write(familyID + "," + trioPileup.getReferenceName() + "," + trioPileup.getPosition() + ","
						+ trioPileup.getRefAllele() + ',' + trioPileup.getAltAllele() + ',' + featureSelection.extract()
						+ "\n");
				numOfAllSites++;
			}
			for (int i = 0; i < 3; i++) {
		...
		try {
			...
			code.addRCode(
					"gbmr <- gbm(Class~.,data=training[,c(selectedFeatures,\"Class\")],shrinkage=0.001,distribution='bernoulli',cv.folds=10,n.trees=50000,interaction.depth = 1, bag.fraction=0.5, verbose=F)");
		...
		logger.info((count - numOfAllSites) + " non-informative candidate DNMs were skipped.");
		logger.info(numOfAllSites + " informative candidate DNMs in " + numOfTrios + " trios were processed.");
		logger.info("Number of true DNMs is " + numOfTrueDNMs + "," + " Number of false positive DNMs is "
				+ (numOfAllSites - numOfTrueDNMs) + ".");
		return true;
	}
